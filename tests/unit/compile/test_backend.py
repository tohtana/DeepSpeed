...
def test_backend_failure_removes_failed_graph_state_before_retry():
    failed_gm = torch.fx.symbolic_trace(lambda x: x + 1)
    failed_gm.meta["dynamo_compile_id"] = SimpleNamespace(frame_id=17)
    failed_graph_id = id(failed_gm.graph)
    surviving_graph_id = failed_graph_id + 1

    backend_mod.graph_order_with_frame_id.clear()
    backend_mod.graph_order_with_frame_id.add_graph(failed_graph_id, 17)
    backend_mod.graph_order_with_frame_id.set_needs_backward(17, True)
    backend_mod.graph_order_with_frame_id.add_graph(surviving_graph_id, 18)
    backend_mod.graph_order_with_frame_id.set_needs_backward(18, False)
    backend_mod.profiling_results.clear()
    backend_mod.profiling_results.update({failed_graph_id: object(), surviving_graph_id: object()})
    backend_mod.param_manager.clear()
    backend_mod.param_manager.update({failed_graph_id: object(), surviving_graph_id: object()})
    backend_mod.frames_partitioned.clear()
    backend_mod.frames_partitioned.update((17, 18))
    backend_mod.fwd_real_inputs[:] = [(torch.ones(1), )]
    ...
    try:
        ...
        assert list(backend_mod.graph_order_with_frame_id.frames) == [18]
        assert failed_graph_id not in backend_mod.profiling_results
        assert failed_graph_id not in backend_mod.param_manager
        assert surviving_graph_id in backend_mod.profiling_results
        assert surviving_graph_id in backend_mod.param_manager
        assert backend_mod.frames_partitioned == {18}
        assert backend_mod.fwd_real_inputs == []

        retry_graph_id = failed_graph_id + 2
        backend_mod.graph_order_with_frame_id.add_graph(retry_graph_id, 17)
        backend_mod.graph_order_with_frame_id.set_needs_backward(17, True)
        assert backend_mod.graph_order_with_frame_id.get_graph_order() == [(surviving_graph_id, False),
                                                                           (retry_graph_id, True)]
    finally:
        backend_mod.graph_order_with_frame_id.clear()
        backend_mod.profiling_results.clear()
        backend_mod.param_manager.clear()
        backend_mod.frames_partitioned.clear()
        backend_mod.fwd_real_inputs.clear()
