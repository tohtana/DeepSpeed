...
class CommDoubleBuffer:
    def __init__(self, dtype: torch.dtype, buffer_size: int, device: torch.device) -> None:
        self.buckets: List[ReduceBucket] = [ReduceBucket(dtype, buffer_size, device),
                                            ReduceBucket(dtype, buffer_size, device)]
        self.events = [torch.cuda.Event(enable_timing=False, blocking=False),
...
class ParamUpdateGroupContainer:
    ...
    def _init_param_update_buffers(self, param_group: Dict[str, Any]) -> None:
        ...
        per_rank_padded_elems, total_padded_elems = sharded_counts_and_totals(
            total_numel, param_group_dtype(param_group), self.world_size)
        ...
        grad_acc_buffer = torch.empty(per_rank_padded_elems,
        ...
        if self.optimizer_dtype is None or self.optimizer_dtype == param_group_dtype(param_group):
            param_for_optimizer = param_buffer[self.rank * per_rank_padded_elems:(self.rank + 1) * per_rank_padded_elems]
        else:
            param_for_optimizer = torch.empty(per_rank_padded_elems, dtype=self.optimizer_dtype, device=self.device)

        return ParamUpdateBuffers(
...
class UniversalOptimizer:
    ...
    def __init__(self, 
        ...
        self.prediv_gradient: bool = gradient_predivide_factor != 1.0
        self.reduce_op = torch.distributed.ReduceOp.SUM if self.prediv_gradient else torch.distributed.ReduceOp.AVG
        ...
        self.rs_stream = torch.cuda.Stream(priority=-1)
        self.copy_stream = torch.cuda.Stream(priority=-1)

        self.rs_comp_done_events = defaultdict(torch.cuda.Event)
        self.rs_copy_done_events = defaultdict(torch.cuda.Event)
    ...
    def gradient_hook(self, param):
        ...
        self.reduce_tasks[param.dtype].append(ReduceTask(copy_src, reduce_in_buffer))
    ...
    def flush_reduce_bucket(self, dtype: torch.dtype):
        ...
        self._apply_pre_division(dtype)
        ...
        self.rs_copy_done_events[dtype].record(self.rs_stream)

        self.perform_cleanup(dtype)
    ...
    def gradient_hook_epilogue(self):
        # flash all
        raise NotImplementedError("gradient_hook_epilogue is not implemented yet.")
    ...
    def _create_comm_buffers(self, reduce_bucket_size: int) -> Dict[torch.dtype, CommDoubleBuffer]:
        ...
        for dtype in dtypes:
            self.comm_buffers[dtype] = CommDoubleBuffer(
                dtype=dtype,
    ...
    def _block_copy_events(self, dtype: torch.dtype):
        ...
        for t in self.reduce_tasks[dtype]:
            ...
            copy_done_event.wait(self.rs_stream)
...
def configure_universal_optimizer(optimizer: torch.optim.Optimizer,
                        config: UniversalOptimizerConfig):
