...
class SequenceStateNode:
    ...
    def __init__(self, block_addresses: List = [],
...
class PrefixCacheTree():
    ...
    def __init__(self, block_size):
        ...
        self.tokens_to_blocks: Dict[str, SequenceStateNode] = {}
        return
    ...
    def lookup(self, tokens: torch.Tensor, speculate: bool = False):
        ...
        for i in range(n_blocks):
            ...
            if current_state_node not in self.tokens_to_blocks:
                break
            cached_blocks = current_node.block_addresses
            if not speculate:
                current_node.visit(self._tree_universal_time)
        return cached_blocks
        
    def extend(self, tokens: torch.Tensor, new_block_ids: List[int], num_already_cached_blocks: int, speculate: bool = False):
        self._update_time()
        n_blocks = len(tokens) // self.block_size
        if num_already_cached_blocks == 0:
            current_node = self.root
        else:
            current_node = self.tokens_to_blocks[sequence_to_state(tokens[:num_already_cached_blocks])]
        
        for i in range(num_already_cached_blocks, n_blocks):
            chunk = tokens[:(i+1)*self.block_size]
            current_state_node = sequence_to_state(chunk)
            if current_state_node not in current_node.children:
                current_node.children[current_state_node] = SequenceStateNode(block_addresses = current_node.block_addresses +[new_block_ids[i]])
    ...
    def delete(self, tokens: torch.Tensor) -> None:
        ...
        for i in range(n_blocks):
            ...
            if current_state_node in current_node.children:
                current_node = current_node.children[current_state_node]
                elimination_stack.append(current_node)
    ...
    def evict(self, num_blocks: int = 1):
        len_eviction_pool = sum((len(i) for i in self.eviction_pool))
        if num_blocks <= len_eviction_pool:
            i = 0
            while len(self.eviction_pool[i]) <= num_blocks:
                for node in self.eviction_pool[i]:
                    del self.tokens_to_blocks[node]
                    
                num_blocks = num_blocks - len(self.eviction_pool[i])

        else:
            raise ValueError(f"Insufficient number of blocks available to be evicted, currently only {len_eviction_pool} blocks can be evicted.")
