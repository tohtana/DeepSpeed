...
def selective_gather(gm: GraphModule, graph_id: int, graph_order: List[Tuple[int, bool]], profiling_results,
    ...
    for profile_graph_id, prof in profiling_results.items():
        ...
        print_rank_0(
            f"selective_gather graph_id={profile_graph_id} "
    ...
    for ds_id in ds_ids:
        ...
        print_rank_0(f"Set persistent: {ds_id} size: {size} persistent_mem: {persistent_mem} shape: {param_obj.ds_shape}")

    if selected_count == 0:
        smallest_candidate = min(ds_id_to_size[ds_id] for ds_id in ds_ids)
        print_rank_0(
            f"selective_gather selected no new params: available_mem={available_mem} "
